...
import { UIComponentType } from "@modelcontextprotocol/sdk/types.js";
...
async function main() {
  ...
  console.log("\n🔧 Creating MCP client...");
  const supportedComponents: UIComponentType[] = [
  ...
  try {
    ...
    if (dashboardResponse.content && Array.isArray(dashboardResponse.content) && dashboardResponse.content[0]?.type === "ui") {
      const uiComponent = (dashboardResponse.content[0] as any).component;
      ...
      if (uiComponent.children && Array.isArray(uiComponent.children)) {
        ...
        uiComponent.children.forEach((child: any, index: number) => {
    ...
    if (formResponse.content && Array.isArray(formResponse.content) && formResponse.content[0]?.type === "ui") {
      const formComponent = (formResponse.content[0] as any).component;
      ...
      if (formComponent.children && Array.isArray(formComponent.children)) {
        ...
        let fieldCount = 0;
        formComponent.children.forEach((child: any, index: number) => {
    ...
    if (salesDataResponse.structuredContent) {
      const structuredContent = salesDataResponse.structuredContent as any;
      ...
      if (structuredContent.data && Array.isArray(structuredContent.data) && structuredContent.data.length > 0) {
        ...
        // Show data summary
        const totalRevenue = structuredContent.data.reduce((sum: number, item: any) => sum + (item.revenue || 0), 0);
        const totalOrders = structuredContent.data.reduce((sum: number, item: any) => sum + (item.orders || 0), 0);
    ...
    if (userDataResponse.structuredContent) {
      const structuredContent = userDataResponse.structuredContent as any;
      ...
      if (structuredContent.data && Array.isArray(structuredContent.data) && structuredContent.data.length > 0) {
        ...
        const roles = structuredContent.data.map((user: any) => user.role).filter(Boolean);
    ...
    if (metricsDataResponse.structuredContent) {
      const structuredContent = metricsDataResponse.structuredContent as any;
    ...
    if (minimalDashboard.content && Array.isArray(minimalDashboard.content) && minimalDashboard.content[0]?.type === "ui") {
      const component = (minimalDashboard.content[0] as any).component;
      ...
      const fullDashboardChildren = dashboardResponse.content?.[0]?.type === "ui" ? 
        (dashboardResponse.content[0] as any).component.children?.length : 0;
